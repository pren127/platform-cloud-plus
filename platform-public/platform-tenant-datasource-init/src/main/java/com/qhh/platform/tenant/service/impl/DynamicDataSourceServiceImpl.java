...
public class DynamicDataSourceServiceImpl implements DataSourceService {
    ...
    @Value("${platform.mysql.database}")
    private String defaultDatabase;
    @Value("${platform.mysql.driverClassName}")
    private String driverClassName;
    @Value("${platform.mysql.username}")
    private String username;
    @Value("${platform.mysql.password}")
    ...
    public Set<String> remove(String name) {
        // 这里每个服务删除的数据库是自己服务配置的 （注意： 必须remote模式，自己按照platform_base_{TENANT}的格式来制定数据库，否则无法删除成功）
    ...
    public boolean initConnect(DataSourcePropertyDTO dto) {
        if (TenantConnectTypeEnum.LOCAL.eq(dto.getType())) {
            // 创建 库
            this.createDatabase(dto.getPoolName());
     ...
     * @param tenant 明文租户编码
     */
    private void createDatabase(String tenant) {
     ...
     * @param runner 脚本运行时
     */
    private void initTables(ScriptRunner runner) {
        try {
     ...
     * @param runner 脚本运行时
     */
    private void initData(ScriptRunner runner) {
        try {
    ...
    private ScriptRunner getScriptRunner(String tenant) {
        try {
            DynamicRoutingDataSource ds = (DynamicRoutingDataSource) this.dataSource;
