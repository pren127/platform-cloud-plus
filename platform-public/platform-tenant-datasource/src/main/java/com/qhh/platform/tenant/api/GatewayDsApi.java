 ...
 * <p>
 * 若你使用的是zuul:
 * 1. 将下面的 platform-gateway-server 改成 platform-zuul-server ！！！！
 * 2. 将path 改成 /api/gate/ds
 *
 * @author qhh
 * @date 2020年04月05日18:18:26
 */
//@FeignClient(name = "${platform.feign.gateway-server:platform-zuul-server}", path = "/api/gate/ds")
@FeignClient(name = "${platform.feign.gateway-server:platform-gateway-server}", path = "/ds")
